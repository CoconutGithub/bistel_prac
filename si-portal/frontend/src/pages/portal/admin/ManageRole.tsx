...
const ManageRole: React.FC = () => {
          ...
          const updatedColumnDefs: any = columnDefs.map((col) => {
            if (col.field === "roleName") {
              return {
                ...col,
                cellEditorParams: {
                  values: res.data.map((item: Role) => item.roleName),
                },
              };
            }
            return col;
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        "http://localhost:8080/admin/api/get-roles",
        {
    ...
    async (lists: { deleteList: any[]; updateList: any[] }) => {
      ...
      try {
        ...
        const response = await axios.post<SaveRolesResponse>(
          "http://localhost:8080/admin/api/save-roles",
          payload,
          {
            headers: { Authorization: `Bearer ${cachedAuthToken}` },
          }
  ...
  const roleRegistButton = useMemo(
    () => (
      <ComButton className="me-2" onClick={handleRegist}>
        권한 추가
      </ComButton>
    ),
    []

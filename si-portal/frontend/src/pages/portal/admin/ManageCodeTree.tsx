...
const ChooseCodeTree: React.FC<ChooseCodeTreeProps> = ({ onCodeClick, refreshTree }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
        try {
            ...
            const res = await axios.get("http://localhost:8080/admin/api/get-code-tree",
                {
...
useEffect(() => {
  ...
  const handleDeleteMenu = async () => { // 자식 노드까지 다 삭제 alert Y, N
    if (contextMenu.node) {
        try {
            ...
            const res = await axios.post("http://localhost:8080/admin/api/delete-code", data, {
                headers: {
  ...
  const handleBlur = async () => {
    ...
    if(inputText.length === 0) {
      ...
      try {
          comAPIContext.showProgressBar();
          const res = await axios.post('http://localhost:8080/admin/api/insert-code', data, {
              headers: {
                  Authorization: `Bearer ${cachedAuthToken}`,
              },
          });

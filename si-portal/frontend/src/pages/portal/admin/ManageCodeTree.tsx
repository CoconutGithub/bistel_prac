...
const ManageCodeTree: React.FC<ChooseCodeTreeProps> = ({ onCodeClick, refreshTree }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
        try {
            ...
            const res = await axios.get(`${process.env.REACT_APP_BACKEND_IP}/admin/api/get-code-tree`,
                {
            ...
            if (res && res.data) {
                ...
                res.data.forEach((e: CodeItem) => {
                    ...
                    e.defaultText = e.msgId != null && e.msgId > -1 ? comAPIContext.$msgDefault(e.msgId): e.defaultText
                    console.log('after : ', e)
                })
...
useEffect(() => {
  ...
  const handleCodeClick = useCallback((event: React.MouseEvent, node: CodeItem) => {
  ...
  const handleDeleteMenu = async () => { // 자식 노드까지 다 삭제 alert Y, N
    if (contextMenu.node) {
        try {
            ...
            const res = await axios.post(`${process.env.REACT_APP_BACKEND_IP}/admin/api/delete-code`, data, {
                headers: {
  ...
  const handleBlur = async () => {
    ...
    if(inputText.length === 0) {
      ...
      try {
          comAPIContext.showProgressBar();
          const res = await axios.post(`${process.env.REACT_APP_BACKEND_IP}/admin/api/insert-code`, data, {
              headers: {
                  Authorization: `Bearer ${cachedAuthToken}`,
              },
          });
  ...
  return (
    <div>
      ...
      {contextMenu.visible && contextMenu.node && (
        ...
        >
            <Dropdown.Header>
                {codeData.find((item) => item.codeId === contextMenu?.node?.codeId)?.defaultText || '/'}
            </Dropdown.Header>
            <Dropdown.Item

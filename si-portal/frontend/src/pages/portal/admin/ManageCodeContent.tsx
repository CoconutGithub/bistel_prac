...
const ManageCodeContent: React.FC<{
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await axios.post(`${process.env.REACT_APP_BACKEND_IP}/admin/api/update-code`, data, {
          headers: {
    ...
    <Container fluid className="p-4">
      {chooseCodeData && chooseCodeData.defaultText !== "Root" ? (
        <>
          ...
          {/* 모달 컴포넌트 */}
          <ManageMessage show={showModal} onClose={handleModalClose} isModal={true} />       
        </>
      ) : (
        <div>
